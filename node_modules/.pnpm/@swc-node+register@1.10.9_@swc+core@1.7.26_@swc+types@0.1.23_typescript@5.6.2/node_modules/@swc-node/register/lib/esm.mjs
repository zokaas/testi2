...
const TSCONFIG_PATH = (function () {
    var _a, _b;
    const pathFromEnv = (_b = (_a = process.env.SWC_NODE_PROJECT) !== null && _a !== void 0 ? _a : process.env.TS_NODE_PROJECT) !== null && _b !== void 0 ? _b : (0, node_path_1.join)(process.cwd(), 'tsconfig.json');
    if (!(0, node_path_1.isAbsolute)(pathFromEnv)) {
        return (0, node_path_1.join)(process.cwd(), pathFromEnv);
    }
    return pathFromEnv;
})();
...
const getPackageForFile = async (url) => {
    ...
    while (true) {
        const path = (0, node_url_1.fileURLToPath)(packageJsonURL);
...
const resolve = async (specifier, context, nextResolve) => {
    ...
    const parsedUrl = (0, node_url_1.parse)(specifier);
    ...
    if (!context.parentURL || parsedUrl.protocol === 'file:') {
        ...
        let format = null;
        const specifierPath = (0, node_url_1.fileURLToPath)(specifier);
        const ext = (0, node_path_1.extname)(specifierPath);
        if (ext === '.js') {
    ...
    const { error, path, moduleType } = await resolver.async((0, node_path_1.join)((0, node_url_1.fileURLToPath)(context.parentURL), '..'), specifier.startsWith('file:') ? (0, node_url_1.fileURLToPath)(specifier) : specifier);
    if (error) {
    ...
    if (path && isPathNotInNodeModules(path)) {
        ...
        const url = new URL((0, node_path_1.join)('file://', path));
        return addShortCircuitSignal({
            ...context,
            url: url.href,
    ...
    try {
        ...
        try {
            const resolution = (0, node_url_1.pathToFileURL)((0, node_module_1.createRequire)(process.cwd()).resolve(specifier)).toString();
